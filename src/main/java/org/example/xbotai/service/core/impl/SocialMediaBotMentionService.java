...
public class SocialMediaBotMentionService {
    ...
    private void processSingleTweet(JsonNode tweet) {
        ...
        // 2. Filter by author: if the tweet's author is not in the DB, the bot skips it.
        ...
        Optional<SocialAccount> accountOpt = socialAccountRepository.findByUserId(tweetAuthorId);
        if (accountOpt.isEmpty()) {
            logger.info("Ignoring tweet because author [{}] is not a registered user.", tweetAuthorId);
            return;
        }
        ...
        try {
            if (text.toLowerCase().contains("trends") && text.contains(botMention)) {
                ...
                try {
                    handleCountryTrendRequest(tweetId, "USA", userId);
                    handled = true;
                } catch (Exception e) {
